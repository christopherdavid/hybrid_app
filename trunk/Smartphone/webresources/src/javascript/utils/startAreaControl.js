function StartAreaControl(startArea, categoryArea, startBtn, categoryTable, categoryCells) {
	...
	this.updateCategoryBtnState = function(element, isMouseDown) {
		...
		if (element.attr('aria-disabled')) {
			categoryBorder.removeClass();
			categoryGradient.removeClass();

			categoryGradient.addClass("categoryBtnGradientDisabled");
			categoryBorder.addClass("categoryBtnBorderDisabled");
	...
	this.updateBtnState = function(divElement, isMouseDown) {
		...
		// Set the correct state when disabled
		if (divElement.attr('aria-disabled')) {
			
			that.startBtnDown = false;

			if (divElement.attr('aria-busy')) {
				divElement.addClass("img-startstopbtn stopbtn-disabled");
			} else {
				divElement.addClass("img-startstopbtn startbtn-disabled");
			}
		} else {

			// Mouse Button down triggered
			if (isMouseDown) {
				that.startBtnDown = true;
				
				if (divElement.attr('aria-busy')) {
					divElement.addClass("img-startstopbtn stopbtn-down");
				} else {
					divElement.addClass("img-startstopbtn startbtn-up");
				}
			} else {
				that.startBtnDown = false;
				
				if (divElement.attr('aria-busy')) {
					divElement.addClass("img-startstopbtn stopbtn-up");
				} else {
					divElement.addClass("img-startstopbtn startbtn-up");
				}
			}
