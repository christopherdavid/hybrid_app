...
public class LogHelper {
	...
	// If sLogEnabled is enabled (by default enabled) then logging is done based on the
	// log level enabled for the device. Log level can be changed using 
	// adb shell setprop log.tag.<YOUR_LOG_TAG> <LEVEL>
	// by default log level is Log.INFO
	// If sEnableDebugLog is specified, then we log everything irrespective of the level set on the
	// device
	...
	// Idea is, individual classes can have their own tags and we can analyze the 
	// logs based on that individual tags but to pick all the logs which are application specified
	// we prefix with tag prefix. This will help to filter all logs which are application specific
	public static void setTagPrefix(String prefix)
	{
		TAG_PREFIX = prefix;
	}
	
	// Private helper method to get the tag which will be used with Log.x APIs. This simply
	...
	// Private helper method to determine, if we should log for the particular level
	// If debug is enabled, then we log everything. Otherwise we log only based on the 
	// level set in the device using  adb shell setprop log.tag.<YOUR_LOG_TAG> <LEVEL>
	...
	// Public helper method to log the debug logs. This logging will be switched off before shipping
	// If you want to log the information even in the shipped product, then call "log" method
	...
	// Public helper method to log information in logcat. This logging will *not* be switched off before shipping
	// Use this method to log only the important information, which you want to keep it even after the app ships
