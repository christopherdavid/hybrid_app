...
public class IsRobotScheduleEnabledRequest extends RobotManagerRequest {
    ...
    private void isScheduleEnabled(final Context context, RobotJsonData jsonData, final String callbackId) {
        ...
        int scheduleTypeOnServer = SchedulerConstants.convertToServerConstants(scheduleType);
        RobotSchedulerManager.getInstance(context).isScheduleEnabled(robotId, scheduleTypeOnServer,
                new RobotRequestListenerWrapper(callbackId) {
                    ...
                    public JSONObject getResultObject(NeatoWebserviceResult responseResult) throws JSONException {
                        ...
                        if ((responseResult != null) && (responseResult instanceof GetRobotProfileDetailsResult2)) {
                            ...
                            String scheduleState = RobotProfileDataUtils
                                    .getScheduleState(context, scheduleType, result);
                            ...
                            jsonResult = getErrorJsonObject(ErrorTypes.ERROR_TYPE_UNKNOWN,
                                    "response result is not of type is schedule enabled result");
                        }
                        return jsonResult;
                    }
                });
