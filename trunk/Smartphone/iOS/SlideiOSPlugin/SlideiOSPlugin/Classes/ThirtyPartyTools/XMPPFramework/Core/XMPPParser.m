...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq parserQueue:(dispatch_queue_t)pq
{
	if ((self = [super init]))
	{
        ...
        IsOnParserQueue = &IsOnParserQueue;
        
        void *nonNullUnusedPointer = (__bridge void *)self;
        dispatch_queue_set_specific(parserQueue, IsOnParserQueue, nonNullUnusedPointer, NULL);
	}
	return self;
}
...
- (void)setDelegate:(id)newDelegate delegateQueue:(dispatch_queue_t)newDelegateQueue
{
	...
	if (dispatch_get_specific(IsOnParserQueue))
		block();
	else
