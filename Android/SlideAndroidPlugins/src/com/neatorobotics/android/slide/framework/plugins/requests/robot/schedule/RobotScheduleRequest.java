...
public class RobotScheduleRequest extends RobotManagerRequest {
    ...
    private class CreateScheduleRequest extends RobotManagerRequest {
        ...
        private void createSchedule(Context context, RobotJsonData jsonData, final String callbackId) {
            ...
            RobotSchedulerManager.getInstance(context).createSchedule(robotId, scheduleType,
    ...
    private class GetScheduleEventDataRequest extends RobotManagerRequest {
        ...
        private void getScheduleEventData(Context context, RobotJsonData jsonData, final String callbackId) {
            ...
            RobotSchedulerManager.getInstance(context).getScheduleEventData(scheduleId, scheduleEventId,
                    new ScheduleRequestListenerWrapper(callbackId));
        }
    ...
    private class UpdateScheduleEventRequest extends RobotManagerRequest {
        private void updateScheduleEvent(Context context, RobotJsonData jsonData, final String callbackId) {
            ...
            ScheduleEvent scheduleEvent = ScheduleJsonDataHelper.jsonToSchedule(event, scheduleEventId, scheduleType);
            RobotSchedulerManager.getInstance(context).updateScheduleEvent(scheduleEvent, scheduleId,
                    new ScheduleRequestListenerWrapper(callbackId));
        }
    ...
    private class DeleteScheduleEventRequest extends RobotManagerRequest {
        private void deleteScheduleEvent(Context context, RobotJsonData jsonData, final String callbackId) {
            final String scheduleId = jsonData.getString(JsonMapKeys.KEY_SCHEDULE_ID);
            final String scheduleEventId = jsonData.getString(JsonMapKeys.KEY_SCHEDULE_EVENT_ID);
            RobotSchedulerManager.getInstance(context).deleteScheduleEvent(scheduleId, scheduleEventId,
                    new ScheduleRequestListenerWrapper(callbackId));
        }
    ...
    private class AddScheduleEventRequest extends RobotManagerRequest {
        private void addScheduleEvent(Context context, RobotJsonData jsonData, final String callbackId) {
            ...
            int scheduleType = getScheduleTypeFromId(context, scheduleId);
            ScheduleEvent scheduleEvent = ScheduleJsonDataHelper.jsonToSchedule(event, eventId, scheduleType);
            RobotSchedulerManager.getInstance(context).addScheduleEvent(scheduleEvent, scheduleId,
                    new ScheduleRequestListenerWrapper(callbackId));
        }
    ...
    private class GetScheduleDataRequest extends RobotManagerRequest {
        private void getScheduleData(Context context, RobotJsonData jsonData, final String callbackId) {
            String scheduleId = jsonData.getString(JsonMapKeys.KEY_SCHEDULE_ID);
            RobotSchedulerManager.getInstance(context).getSchedule(scheduleId,
                    new ScheduleRequestListenerWrapper(callbackId));
        }
    ...
    private class UpdateScheduleRequest extends RobotManagerRequest {
        ...
        private void updateSchedule(final Context context, RobotJsonData jsonData, final String callbackId) {
            final String scheduleId = jsonData.getString(JsonMapKeys.KEY_SCHEDULE_ID);
            RobotSchedulerManager.getInstance(context).addUpdateSchedule(scheduleId,
                    new ScheduleRequestListenerWrapper(callbackId) {
                        @Override
                        public void onScheduleData(JSONObject scheduleJson) {
                            ...
                            // Passing NoActionWebServiceRequestListener in
                            // listener because we don't want to
                            // send the response to the UI layer
                            // TODO:
                            // However this has one issue. If Schedule is
                            // updated but the API to tell the robot that
                            // schedule is updated
                            // fails then robot will not know that scheduled has
                            // changed. Either server should send the message
                            // to the robot whenever schedule is updated or if
                            // this API fails, SmartApp needs to save this
                            // information
                            // and try again
                            RobotDataManager.sendRobotScheduleUpdated(context, robotId,
                                    new NoActionWebServiceRequestListener());
                        }
                    });
        }
    ...
    private int getScheduleTypeFromId(Context context, String id) {
        ...
        return SchedulerConstants.getScheduleIntType(scheduleType);
    }
