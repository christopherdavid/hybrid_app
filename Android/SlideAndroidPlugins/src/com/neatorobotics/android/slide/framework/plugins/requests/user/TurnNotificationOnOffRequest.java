...
public class TurnNotificationOnOffRequest extends UserManagerRequest {
    ...
    private void turnNotificationOnOff(final Context context, final UserJsonData jsonData, final String callbackId) {
        ...
        LogHelper.logD(TAG,
                String.format("Email = %s NotificationId = %s Enable = %s", email, notificationId, onOffFlag));

        SettingsManager.getInstance(context).updateNotificationState(email, notificationId, onOffFlag,
