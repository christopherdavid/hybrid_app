...
public class TurnWifiOnOffRequest extends RobotManagerRequest {

	@Override
	public void execute(String action, JSONArray data, String callbackId) {
		RobotJsonData jsonData = new RobotJsonData(data);
		turnWiFiOnOff(mContext, jsonData, callbackId);
	}
	private void turnWiFiOnOff (final Context context, RobotJsonData jsonData, final String callbackId) {
		...
		LogHelper.logD(TAG, "sendCommand2 - turnWiFiOnOff called");
		RobotDataManager.sendRobotCommand(context, robotId, RobotCommandPacketConstants.COMMAND_TURN_WIFI_ONOFF, commadParamsMap  , new RobotSetProfileDataRequestListener(callbackId));
	}
}
