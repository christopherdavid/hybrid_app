...
public class UserManager extends Observable {
    ...
    public void resendValidationMail(final String email, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            ...
            public void run() {
                try {
                    ResendValidationMailResult resendValidationMailResult = NeatoUserWebservicesHelper
                            .resendValidationMailResult(mContext, email);
    ...
    public void getUserDetails(final String email, final String auth_token, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    GetNeatoUserDetailsResult result = NeatoUserWebservicesHelper.getNeatoUserDetails(mContext, email,
                            auth_token);
    ...
    public void loginUser(final String email, final String password, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    LoginNeatoUserTokenResult loginResult = NeatoUserWebservicesHelper.loginNeatoUserToken(mContext,
                            email, password);
    ...
    public void createUser3(final String username, final String email, final String alternateEmail,
        ...
        Runnable task = new Runnable() {
            ...
            public void run() {
                try {
                    CreateNeatoUserResult createUserResult = NeatoUserWebservicesHelper.createNeatoUser3RequestNative(
                            mContext, username, email, alternateEmail, password, userParams);
                    LogHelper.logD(TAG, "user validation status is " + createUserResult.result.validation_status);
                    GetNeatoUserDetailsResult userDetailsResult = NeatoUserWebservicesHelper.getNeatoUserDetails(
                            mContext, email, createUserResult.result.user_handle);
                    UserHelper.saveLoggedInUserDetails(mContext, userDetailsResult.result,
    ...
    public void associateRobot(final String robotId, final String emailId, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    RobotAssociationDisassociationResult result = NeatoUserWebservicesHelper
                            .associateNeatoRobotRequest(mContext, emailId, robotId);
    ...
    public void disassociateRobot(final String robotId, final String emailId,
            final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    RobotAssociationDisassociationResult result = NeatoUserWebservicesHelper
                            .disassociateNeatoRobotRequest(mContext, emailId, robotId);
    ...
    public void disassociateAllRobots(final String email, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    RobotAssociationDisassociationResult result = NeatoUserWebservicesHelper
                            .dissociateAllNeatoRobotsRequest(mContext, email);
    ...
    public void getAssociatedRobots(final String email, final String authToken,
            final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    GetUserAssociatedRobotsResult result = NeatoUserWebservicesHelper.getUserAssociatedRobots(mContext,
                            email, authToken);
    ...
    public void changePassword(final String authToken, final String currentPassword, final String newPassword,
            final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            @Override
            public void run() {
                try {
                    ChangePasswordResult result = NeatoUserWebservicesHelper.changePasswordRequest(mContext, authToken,
        ...
        Runnable task = new Runnable() {
            public void run() {
                try {
                    ...
                    attributes.put(NeatoUserWebServicesAttributes.SetUserAttributes.Attribute.ATTRIBUTE_NAME,
                            userAttributes.deviceName);
                    attributes.put(
                            NeatoUserWebServicesAttributes.SetUserAttributes.Attribute.ATTRIBUTE_OPERATING_SYSTEM,
                            userAttributes.osName);
                    attributes.put(NeatoUserWebServicesAttributes.SetUserAttributes.Attribute.ATTRIBUTE_VERSION,
                            userAttributes.osVersion);
                    NeatoUserWebservicesHelper.setUserAttributeRequest(mContext, authToken, attributes);
                } catch (UserUnauthorizedException ex) {
                    LogHelper.log(TAG,
                            "UserUnauthorizedException in setUserAttributesOnServer - " + ex.getErrorMessage());
                } catch (NeatoServerException ex) {
    ...
    public void setUserAccountDetails(final String email, final String authToken, final String countryCode,
            final String optIn, final WebServiceBaseRequestListener listener) {
        Runnable task = new Runnable() {
            public void run() {
                try {
                    HashMap<String, String> attributes = new HashMap<String, String>();
                    attributes.put(
                            NeatoUserWebServicesAttributes.SetUserAccountDetails.Attribute.ATTRIBUTE_COUNTRYCODE,
                            countryCode);
                    attributes.put(NeatoUserWebServicesAttributes.SetUserAccountDetails.Attribute.ATTRIBUTE_OPTIN,
                            optIn);
                    SetUserAccountDetailsResult result = NeatoUserWebservicesHelper.setUserAccountDetailsRequest(
                            mContext, email, authToken, attributes);
                    listener.onReceived(result);
                } catch (UserUnauthorizedException ex) {
                    listener.onServerError(ErrorTypes.ERROR_TYPE_USER_UNAUTHORIZED, ex.getErrorMessage());
                } catch (NeatoServerException ex) {
