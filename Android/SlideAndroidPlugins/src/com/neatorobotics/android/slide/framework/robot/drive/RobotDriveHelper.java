...
public class RobotDriveHelper {
	...
	public void setRobotDriveRequest (String robotId, WebServiceBaseRequestListener listener) {
		...
		if (driveStatus.isConnectionAllowed()) {
			...
			RobotDataManager.setRobotProfileParam(mContext, robotId, ProfileAttributeKeys.INTEND_TO_DRIVE, driveDetail, listener);
		}
		else {
	...
	public void cancelRobotDriveRequest (String robotId, WebServiceBaseRequestListener listener) {
		...
		// If the user is already connected to the robot VIA direct connection, return with a error.
		if (RobotCommandServiceManager.isRobotDirectConnected(mContext, robotId)) {
			listener.onServerError(ErrorTypes.ROBOT_ALREADY_CONNECTED, "Robot already connected cannot cancel intend to drive");
			return;
		}
		
		if (isRobotDriveRequested(robotId)) {
			LogHelper.logD(TAG, "Robot request exists. Cancelling = " + robotId);
			RobotDataManager.deleteProfileDetailKey(mContext, robotId, ProfileAttributeKeys.INTEND_TO_DRIVE, true, listener);
		}
		else {
	...
	public void driveRobot(String robotId, String navigationControlId) {
		...
		RobotCommandServiceManager.sendCommandToPeer(mContext, robotId, RobotCommandPacketConstants.COMMAND_DRIVE_ROBOT, commandParams);
	}
	...
	public void robotReadyToDrive (final String robotId, String driveRobotIp) {
		if (isRobotDriveRequested(robotId)) {
			...
			RobotCommandServiceManager.tryDirectConnectionWithIp(mContext, robotId, driveRobotIp, new RobotPeerConnectionListener() {
	...
	// This would mean that either the intend to drive is successful or cancelled.
	...
	// Method gives the drive request json value to be set in the profile parameter for intendToDrive.
	...
	// Called before sending intendToDrive to see if the robot drive is allowed or not
	// Currently checks if any direct connection exists with any robot from smartapp
	...
	// Helper method to send plugin notification that robot drive request failed.
