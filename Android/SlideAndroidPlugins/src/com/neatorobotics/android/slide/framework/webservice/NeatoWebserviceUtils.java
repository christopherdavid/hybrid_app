...
public class NeatoWebserviceUtils {
	...
	public static <T extends NeatoWebserviceResult> T readValueHelper(NeatoHttpResponse response, Class<T> responseClassType) {
		T result = null;
	...
	private static <T> T createErrorResponseObject(Class<T> responseClassType) {
		...
		try {
			...
			result = responseClassType.getDeclaredConstructor(cArgs).newInstance(NeatoWebConstants.RESPONSE_SERVER_ERROR, NeatoWebConstants.RESPONSE_SERVER_ERROR_JSON_PARSING, "JSON Exception");
		} 
		catch (InstantiationException e) {
			LogHelper.log(TAG, "Exception in readValueHelper", e);
		} 
