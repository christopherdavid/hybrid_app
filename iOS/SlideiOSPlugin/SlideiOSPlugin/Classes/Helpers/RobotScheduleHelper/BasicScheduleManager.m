...
- (void)start {
    id dbResult = [[NeatoDataStore sharedNeatoDataStore] basicScheduleForScheduleId:self.scheduleId];
    ...
    self.robotId = [[NeatoDataStore sharedNeatoDataStore] robotIdForScheduleId:self.scheduleId];
...
- (void)postedSchedule:(PostScheduleResult *)result {
    ...
    [database updateServerScheduleId:result.serverScheduleId andScheduleVersion:result.scheduleVersion forScheduleWithScheduleId:self.scheduleId];
...
- (void)updatedScheduleWithResult:(id)result {
    ...
    NeatoDataStore *database = [NeatoDataStore sharedNeatoDataStore];
    [database updateScheduleVersion:[resultDict stringForKey:KEY_SCHEDULE_VERSION] forScheduleWithScheduleId:self.scheduleId];
    [self.delegate performSelector:@selector(updatedSchedule:) withObject:self.scheduleId];
     [self notifyScheduleUpdated];
}
...
- (void)gotSchedulesData:(id)scheduleData forRobotId:(NSString *)robotId {
    ...
    if (!scheduleDictionary) {
        ...
        NeatoDataStore *database = [NeatoDataStore sharedNeatoDataStore];
        [database updateServerScheduleId:[scheduleDictionary valueForKey:KEY_ID] andScheduleVersion:[scheduleDictionary valueForKey:KEY_XML_DATA_VERSION] forScheduleWithScheduleId:self.scheduleId];
