...
@interface XMPPStream ()
{
    ...
    void *IsOnXMPPQueueOrTargetQueueKey;
}
...
- (void)commonInit
{
    ...
    IsOnXMPPQueueOrTargetQueueKey = &IsOnXMPPQueueOrTargetQueueKey;
    
    void *nonNullUnusedPointer = (__bridge void *)self;
    dispatch_queue_set_specific(xmppQueue, IsOnXMPPQueueOrTargetQueueKey, nonNullUnusedPointer, NULL);
}
...
- (XMPPStreamState)state
{
	...
	if (dispatch_get_specific(IsOnXMPPQueueOrTargetQueueKey))
		block();
	else
...
- (void)continueSendMessageData:(NSData *)data withTag:(long)tag
{
	//XMPPLogSend(@"SEND: %@", outgoingStr);
	...
	[multicastDelegate xmppStream:self didSendMessage:nil];
}
