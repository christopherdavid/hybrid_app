...
- (id)initWithdDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq resolverQueue:(dispatch_queue_t)rq
{
	...
	if ((self = [super init]))
	{
        ...
        IsOnResolveQueue = &IsOnResolveQueue;
        
        void *nonNullUnusedPointer = (__bridge void *)self;
        dispatch_queue_set_specific(resolverQueue, IsOnResolveQueue, nonNullUnusedPointer, NULL);
	}
	return self;
}
...
- (NSString *)srvName
{
	...
	if (dispatch_get_specific(IsOnResolveQueue))
		block();
	else
...
- (void)sortResults
{
	NSAssert(dispatch_get_specific(IsOnResolveQueue), @"Invoked on incorrect queue");
